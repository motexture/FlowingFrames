...
from models.conditioner import TemporalConditionerTransformer
...
class UNetMidBlock3DCrossAttn(nn.Module):
    def __init__(
        ...
        temp_cond_attentions = []
        temp_attentions = []
        attentions = []

        for _ in range(num_layers):
            temp_cond_attentions.append(
                TemporalConditionerTransformer(
        ...
        self.temp_cond_attentions = nn.ModuleList(temp_cond_attentions)
    ...
    def forward(
        ...
        for temp_cond_attn, temp_attn, attn, resnet in zip(self.temp_cond_attentions, self.temp_attentions, self.attentions, self.resnets[1:]):
            if self.gradient_checkpointing:
                ...
                hidden_states = torch.utils.checkpoint.checkpoint(create_custom_forward(temp_cond_attn, return_dict=False), hidden_states, conditioning_hidden_states)[0]
                ...
                hidden_states = temp_cond_attn(hidden_states, conditioning_hidden_states).sample
...
class CrossAttnDownBlock3D(nn.Module):
    ...
    def forward(
        ...
        for resnet, attn, temp_attn, temp_cond_attn in zip(self.resnets, self.attentions, self.temp_attentions, self.temp_cond_attentions):
            if self.gradient_checkpointing:
                def create_custom_forward(module, return_dict=None):
                    def custom_forward(*inputs):
                        if return_dict is not None:
